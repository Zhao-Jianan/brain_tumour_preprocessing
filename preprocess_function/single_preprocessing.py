...
import subprocess, ants
from scipy.ndimage import zoom
from .plot import show_slices
from .logger import logger


def convert_orientation(img: nib.Nifti1Image, target_orientation='AX') -> nib.Nifti1Image:
    """
    将图像从当前方向转换为目标方向（例如 SAG → AX），同时更新 affine，保持 RAS 坐标系不变。
    支持 SAG, AX, COR 三种方向的相互转换。
    """
    data = img.get_fdata()
    ...
    shape = data.shape
    logger.info(f"原始图像 shape: {shape}")
    logger.info(f"原始图像方向: {nib.aff2axcodes(original_affine)}")

    def detect_orientation(shape):
        if shape[0] < shape[1] and shape[0] < shape[2]:
            return 'SAG'
        elif shape[1] < shape[0] and shape[1] < shape[2]:
            return 'COR'
    ...
    current_orientation = detect_orientation(shape)
    logger.info(f"当前图像方向: {current_orientation}")
    
    if current_orientation == target_orientation:
        logger.info("无需转换，图像已经是目标方向")
        return img
    ...
    transpose_order = transpose_map.get((current_orientation, target_orientation))
    if transpose_order is None:
        logger.error(f"不支持的方向转换：{current_orientation} -> {target_orientation}")
        return img

    logger.info(f"应用转置顺序: {transpose_order}")
    transposed_data = np.transpose(data, transpose_order)
    new_shape = transposed_data.shape
    logger.info(f"转置后图像 shape: {new_shape}")

    # 原 spacing
    original_spacing = header.get_zooms()[:3]
    new_spacing = [original_spacing[i] for i in transpose_order]
    logger.info(f"原 spacing: {original_spacing} → 新 spacing: {new_spacing}")

    # 构造新的 affine，保持 RAS 不变
    new_affine = np.eye(4)
    ras_axes = nib.aff2axcodes(original_affine)
    ras_sign = np.sign(np.diag(original_affine)[:3])  # 获取方向正负

    for i, axis in enumerate(transpose_order):
        new_affine[i, i] = new_spacing[i] * ras_sign[axis]
        new_affine[i, 3] = original_affine[axis, 3]  # 保留原位置（可以根据需要修改）

    logger.info(f"更新 affine 完成，new affine:\n{new_affine}")
    logger.info(f"  当前方向: {nib.aff2axcodes(new_affine)}")

    return nib.Nifti1Image(transposed_data, new_affine, header)
...
def load_and_convert_to_ras(input_path):
    ...
    transposed_img = convert_orientation(img)
    data = transposed_img.get_fdata()
    affine = transposed_img.affine

    current_ornt = nib.aff2axcodes(affine)
    logger.info(f"  当前方向: {current_ornt}")

    if current_ornt != ('R', 'A', 'S'):
        logger.info("  转换为 RAS 方向")
        ...
        affine = transposed_img.as_reoriented(transform).affine

        new_ornt = nib.aff2axcodes(affine)
        logger.info(f"  转换后方向: {new_ornt}")

    else:
        logger.info("  已经是 RAS，无需转换")
...
def resample_volume(data, affine):
    ...
    logger.info(f"原始体素尺寸: {np.sqrt(np.sum(affine[:3,:3]**2, axis=0))} mm")
    ...
    logger.info(f"自动计算输出尺寸: {data.shape} -> {output_shape}")
    ...
    logger.info(f"重采样完成。实际体素尺寸: {final_voxel} mm")
...
def multimodal_alignment(moving_data, fixed_data, moving_affine, fixed_affine):
    ...
    logger.info("Performing multimodal alignment...")
...
def pad_or_crop_to_target(data, target_shape):
    ...
    for i in range(3):
        ...
        if diff == 0:
            logger.info(f"  维度 {i} 的尺寸相同，无需补零")
            padding.append((0, 0))
        elif diff > 0:
            pad_before = diff // 2
            pad_after = diff - pad_before
            padding.append((pad_before, pad_after))
            logger.info(f"  维度 {i} 补零: 前 {pad_before}，后 {pad_after}")
        else:
            # 如果目标维度小于当前维度，裁剪固定数量
            total_remove = 15
            ...
            logger.info(f"  维度 {i} 裁剪: 前 {remove_before}，后 {remove_after}，裁剪后形状: {padded_or_cropped_data.shape}")
            ...
            if new_diff >= 0:
                ...
                padding.append((pad_before, pad_after))
                logger.info(f"  维度 {i} 再补零: 前 {pad_before}，后 {pad_after}")
            else:
    ...
    logger.info(f"  最终形状: {padded_or_cropped_data.shape}")
...
def n4_bias_correction(input_data, affine):
    ...
    logger.info("使用 SimpleITK 进行 N4 校正...")

    # 将 numpy array 转换为 SimpleITK 图像
    logger.info("Step 1: 转换为 SimpleITK 图像")
...
def run_brain_extraction(input_path, output_prefix='./preprocessed/brain_mask'):
    ...
    # mask 默认输出路径
    bet_path = output_prefix + '.nii.gz'

    cmd = [
        'hd-bet',
        '-i', input_path,
        '-o', bet_path,
        '-device', 'cuda:1',              # 指定使用 CPU；如有 GPU 可改为 'gpu'
        '--no_bet_image',              # 不保存剥除后的脑图像
        '--save_bet_mask',             # 保存脑掩膜（默认命名为 *_mask.nii.gz）
        '--verbose',                # 显示详细信息
    ]
    ...
    try:
        ...
        logger.error(f"HD-BET 执行失败: {e}")


    mask_path = output_prefix + '_bet.nii.gz'   
...
def run_preprocessing_pipeline(input_path, 
    ...
    logger.info("\n步骤3/7: 多模态配准")
    if reference_path is None:
        logger.info("  跳过配准步骤（当前是 T1ce 模态）")
        aligned_data, aligned_affine = resampled, new_affine
    else:
        logger.info("  使用 T1ce 重采样结果作为基准进行配准")
    ...
    show_slices(aligned_data, "After Registration", 
                plot_mode=plot_mode, save_path=plot_save_path, save_name="after_registration.png")
    ...
    show_slices(padded_or_cropped_data, "After Padding/Cropping", 
                plot_mode=plot_mode, save_path=plot_save_path, save_name="after_padding_cropping.png")
    ...
    if skip_n4:
        ...
        show_slices(corrected, "After N4 Correction",
                    plot_mode=plot_mode, save_path=plot_save_path, save_name="after_n4_correction.png")
    ...
    if batch_process:
        ...
        show_slices(normalized, "After Normalization",
                    plot_mode=plot_mode, save_path=plot_save_path, save_name="after_normalization.png")
