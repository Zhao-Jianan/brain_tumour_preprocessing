...
def convert_orientation(img: nib.Nifti1Image, target_orientation='AX') -> nib.Nifti1Image:
    ...
    shape = data.shape
    print(f"[INFO] 原始图像 shape: {shape}")
    print(f"  原始图像方向: {nib.aff2axcodes(original_affine)}")
    ...
    print(f"[INFO] 当前图像方向: {current_orientation}")
    
    if current_orientation == target_orientation:
        print("[INFO] 无需转换，图像已经是目标方向")
        return img
    ...
    if transpose_order is None:
        print(f"[ERROR] 不支持的方向转换：{current_orientation} -> {target_orientation}")
        return img

    print(f"[INFO] 应用转置顺序: {transpose_order}")
    transposed_data = np.transpose(data, transpose_order)
    new_shape = transposed_data.shape
    print(f"[INFO] 转置后图像 shape: {new_shape}")

    # 原 spacing
    original_spacing = header.get_zooms()[:3]
    new_spacing = [original_spacing[i] for i in transpose_order]
    print(f"[INFO] 原 spacing: {original_spacing} → 新 spacing: {new_spacing}")
    ...
    print(f"[INFO] 更新 affine 完成，new affine:\n{new_affine}")
    print(f"  当前方向: {nib.aff2axcodes(new_affine)}")
...
def run_brain_extraction(input_path, output_prefix='./preprocessed/brain_mask'):
    ...
    cmd = [
        ...
        '-i', input_path,
        '-o', bet_path,
        '-device', 'cuda:9',              # 指定使用 CPU；如有 GPU 可改为 'gpu'
